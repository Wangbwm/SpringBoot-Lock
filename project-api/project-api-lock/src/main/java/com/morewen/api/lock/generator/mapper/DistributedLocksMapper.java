...
public interface DistributedLocksMapper extends BaseMapper<DistributedLocks> {
    @Select("SELECT * FROM distributed_locks WHERE lock_name = #{lockName} FOR UPDATE")
    DistributedLocks selectLockForUpdate(String lockName);

    @Update("UPDATE distributed_locks SET locked = #{locked} WHERE id = #{id}")
    int updateLockStatus(DistributedLocks entity);
    @Select("SELECT * FROM distributed_locks WHERE lock_name = #{lockName}")
    DistributedLocks selectForName(String lockName);
    @Update("UPDATE distributed_locks SET locked = #{locked} WHERE id = #{id} AND version = #{currentVersion}")
